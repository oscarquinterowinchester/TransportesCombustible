...
import com.appchoferes.nomina.config.durandb;
...
public class UserService {
    ...
    public boolean authenticateUser(String loginUsuario, String passwordUsuario) {
        ...
        System.out.println("passwordUsuario:" + passwordUsuario);
        System.out.println("Primera consulta: " + sql);

        Map<String, Object> result;

        try {
            System.out.println("SQL de la consulta: " + sql);
            System.out.println(
                    "Parametros de la consulta: loginUsuario=" + loginUsuario + ", passwordUsuario=" + passwordUsuario);
            result = jdbcTemplate.queryForMap(sql, loginUsuario, passwordUsuario);
            System.out.println("Resultado consulta: " + result);
        } catch (Exception e) {
            System.out.println("Error primera query: " + e.getMessage());
            return false;
        }
        ...
        try {
            DataSource userDS = durandbConfig.createUserDataSource(driverClassName, dbUrl, dbUser, dbPass);
        ...
        UserContextHolder
                .setUserContext(new UserContext(dbName, dbUser, dbPass, dbEmpresa, dbNombreUsuario, dbApellidoPaterno));
        return true;
    }
}
